...
import { CalendarIcon, ChevronRightIcon, ClipboardListIcon, PlusIcon, ZapIcon } from "lucide-react"
...
export default function SesionesPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  const role = teams?.find((t) => t.team.id === teamId)?.role
  const isCoach = role === "coach"

  return isCoach
    ? <CoachSessions teamId={teamId} />
    : <AthleteSessions teamId={teamId} />
}

// ─── Coach view ───────────────────────────────────────────────────────────────

function CoachSessions({ teamId }: { teamId: string }) {
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
      ...
      {active.length > 0 && (
        <section className="space-y-2">
          ...
          {active.map((s) => (
            <CoachSessionCard key={s.id} teamId={teamId} session={s} />
          ))}
        </section>
      )}
        ...
        <div className="flex flex-col items-center justify-center py-16 border border-dashed border-border rounded-xl gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Asigna una rutina a tus atletas para comenzar.
            </p>
          </div>
          <Link
            href={`/teams/${teamId}/sesiones/new`}
...
function AthleteSessions({ teamId }: { teamId: string }) {
  const { data: mySessions } = trpc.sessions.mySessions.useQuery({ teamId })

  const pending = mySessions?.filter(
    (s) => s.athleteSessionStatus === "active" && s.sessionStatus === "active"
  ) ?? []
  const done = mySessions?.filter(
    (s) => s.athleteSessionStatus === "completed" || (s.athleteSessionStatus === "active" && s.sessionStatus !== "active")
  ) ?? []
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
      ...
      {pending.length > 0 && (
        <section className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Pendientes</p>
          </div>
          {pending.map((s) => (
            <AthleteSessionCard key={s.athleteSessionId} teamId={teamId} session={s} />
          ))}
        </section>
      )}
      ...
      {mySessions?.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 border border-dashed border-border rounded-xl gap-3 text-center">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">Sin sesiones asignadas</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              Tu entrenador te asignará sesiones aquí.
            </p>
          </div>
        </div>
      )}
...
const SESSION_STATUS_CONFIG = {
  active: { label: "Activa", dot: "bg-green-400", badge: "bg-green-500/10 text-green-400 border-green-500/20" },
...
function CoachSessionCard({ teamId, session }: { teamId: string; session: CoachSession }) {
...
function AthleteSessionCard({ teamId, session }: { teamId: string; session: AthleteSession }) {
  const isCompleted = session.athleteSessionStatus === "completed"
  const isEvaluation = session.sessionType === "evaluation"

  return (
    <Link
      href={`/teams/${teamId}/sesiones/${session.sessionId}`}
      className="group flex items-center gap-3 p-4 border border-border rounded-xl hover:border-primary/30 hover:bg-card bg-card/60 transition-all duration-200 cursor-pointer"
    >
      {/* Status accent bar */}
      <div className={cn(
        "w-1 h-9 rounded-full shrink-0",
        isCompleted ? "bg-emerald-400" : isEvaluation ? "bg-primary" : "bg-primary/50"
      )} />
      ...
      <div className="flex items-center gap-2 shrink-0">
        {isEvaluation && (
          <span className="text-[10px] font-semibold uppercase tracking-wide text-primary border border-primary/30 rounded-md px-1.5 py-0.5">
            Evaluación
          </span>
        )}
        {isCompleted ? (
