...
export default function NewSessionPage() {
  ...
  const { data: routines } = trpc.routines.list.useQuery({ teamId }, { enabled: !!teamId })
  const { data: members } = trpc.teams.members.useQuery({ teamId }, { enabled: !!teamId })
    ...
    <div className="max-w-lg mx-auto px-6 py-10 space-y-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="space-y-3">
          ...
          {athletes.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No hay atletas en este equipo todavía.
            </p>
          )}
          <div className="space-y-2">
            {athletes.map((a) => (
        ...
        >
          {createSession.isPending ? "Iniciando..." : "Iniciar sesión"}
        </button>
      </form>
    </div>
  )
